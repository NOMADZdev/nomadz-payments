...
describe('PER-style booking payment (init + settle)', () => {
  ...
  before(async () => {
    ...
    const destinationVault = new PublicKey(getAccount<string>('configDestinationVault') || '');
    ...
    console.log('Destination vault token account:', destVaultTokenAccount.toBase58());
  ...
  it('init creates/updates BookingPayment PDA; settle moves tokens and marks Settled', async () => {
    ...
    const bookingSeed = createHash('sha256').update(`booking${hotelId}:${userId}`).digest();

    const [bookingPaymentPda] = PublicKey.findProgramAddressSync(
      [Buffer.from('booking_payment'), wallet.publicKey.toBuffer(), Buffer.from(bookingSeed)],
      program.programId,
    );

    const feeVaultBefore =
      await program.provider.connection.getTokenAccountBalance(feeVaultTokenAccount);
    const destVaultBefore =
      await program.provider.connection.getTokenAccountBalance(destVaultTokenAccount);
    ...
    if (typeof bp.status === 'string') {
      assert.strictEqual((bp.status as string).toLowerCase(), 'pending');
    ...
    const feeDelta = Number(feeVaultAfter.value.amount) - Number(feeVaultBefore.value.amount);
    const destDelta = Number(destVaultAfter.value.amount) - Number(destVaultBefore.value.amount);
    ...
    const bpAfter = await program.account.bookingPayment.fetch(bookingPaymentPda);

    if (typeof bpAfter.status === 'string') {
      assert.strictEqual((bpAfter.status as string).toLowerCase(), 'settled');
